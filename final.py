...
while True:
    ...
    cv2.putText(frame, "LINE A", (20, LINE_A_Y - 10),
                cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 255), 2)

    cv2.putText(frame, "LINE B", (20, LINE_B_Y - 10),
    ...
    if vehicle_results and vehicle_results[0].boxes is not None:
        ...
        for box in vehicle_results[0].boxes:
            ...
            if vehicle_crop.size > 0:
                ...
                if plate_results and plate_results[0].boxes is not None:
                    for plate_box in plate_results[0].boxes:
                        ...
                        cv2.rectangle(frame, (real_x1, real_y1),
                                      (real_x2, real_y2), (255, 0, 0), 2)

                        cv2.putText(frame, "plate", (real_x1, real_y1 - 5),
            ...
            cv2.putText(frame, label, (x1, y1 - 10),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.7, vehicle_color, 2)
            ...
            if new_event is not None:
                ...
                if (
                    ...
                    screenshot_path = os.path.join(
                        SCREENSHOT_FOLDER,
                        f"speeding_id{track_id}_{new_event['speed_kmh']:.1f}kmh.jpg"
                    )
                ...
                ws.append([
                    new_event["track_id"],
                    last_plate_text.get(track_id, ""),
                    new_event["direction"],
                    round(new_event["speed_kmh"], 2),
                    SPEED_LIMIT_KMH,
                    new_event["status"],
                    new_event["frame_number"],
                    LINE_A_Y,
                    LINE_B_Y,
                ...
                print(
                    f"Track {track_id} | Plate:{last_plate_text.get(track_id, '')} | "
                    f"{new_event['direction']} | "
                    f"{new_event['speed_kmh']:.2f} km/h | {new_event['status']}"
                )

    display = cv2.resize(frame, (DISPLAY_WIDTH, DISPLAY_HEIGHT))
    cv2.imshow("Final Speed Camera System", display)
